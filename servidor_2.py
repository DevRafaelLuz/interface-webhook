...
MINHA_PORTA = 5002
URL_WEBHOOK_DESTINO = "http://127.0.0.1:5001/webhook"

app = Flask(__name__)

@app.route('/webhook', methods=['POST'])
def receber_webhook():
    global chave_publica_a, modulo_a
    payload = request.get_json()
    tipo = payload.get('tipo')

    if tipo == 'troca_chave':
        chave_publica_a = int(payload['e'])
        modulo_a = int(payload['n'])
...
def loop_envio():
        ...
        try:
            ...
            if resposta.status_code == 200:
                ...
                if 'e' in dados and 'n' in dados:
                    chave_publica_a = int(dados['e'])
                    modulo_a = int(dados['n'])
        except requests.exceptions.ConnectionError:
            pass
        
        if not chave_publica_a:
            time.sleep(1)
    ...
    while True:
        try:
            ...
            num_cifrado = encrypt_message(texto, chave_publica_a, modulo_a)
            requests.post(URL_WEBHOOK_DESTINO, json={"tipo": "mensagem", "conteudo": str(num_cifrado)})
        except Exception as e:
            print(f"[ERRO]: {e}")

if __name__ == '__main__':
    print(f"[*] Iniciando Terminal B na porta {MINHA_PORTA}...")
