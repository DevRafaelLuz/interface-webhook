...
servidor.listen(1)
print("[*] Aguardando conexão do Terminal B...")

conexao, endereco = servidor.accept()
print("[*] Conectado!")

conexao.send(f"{meu_e},{meu_n}".encode('utf-8'))
dados_chave_cliente = conexao.recv(1024).decode('utf-8')
cliente_e, cliente_n = map(int, dados_chave_cliente.split(','))
...
while True:
    try:
        ...
        num_cifrado = encrypt_message(texto, cliente_e, cliente_n)
        conexao.send(str(num_cifrado).encode('utf-8'))
